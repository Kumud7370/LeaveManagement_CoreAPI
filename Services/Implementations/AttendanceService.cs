...
namespace AttendanceManagementSystem.Services.Implementations
{
    public class AttendanceService : IAttendanceService
    {
        ...
        // ── Resolve the Employee record that belongs to the logged-in user ──────
        private async Task<string?> ResolveEmployeeIdFromUserIdAsync(string userId)
        {
            var user = await _userRepository.GetByIdAsync(userId);
            if (user == null)
            {
                _logger.LogWarning("CheckIn/Out: User with id '{UserId}' not found.", userId);
                return null;
            }
            ...
            {
                _logger.LogWarning(
                    "CheckIn/Out: User '{Username}' (id={UserId}) has no linked EmployeeId. " +
                    "Go to Employees → assign this user to an employee record.",
        ...
        public async Task<AttendanceResponseDto?> CheckInAsync(CheckInDto dto, string userId)
        {
            ...
            if (employee == null)
            {
                _logger.LogWarning("CheckIn: Employee record '{EmployeeId}' not found.", employeeId);
                return null;
            }
            ...
            {
                _logger.LogInformation("CheckIn: Employee '{EmployeeId}' already checked in today.", employeeId);
                return null;
            }
        ...
        public async Task<AttendanceResponseDto?> CheckOutAsync(CheckOutDto dto, string userId)
        {
            ...
            {
                _logger.LogWarning("CheckOut: Employee '{EmployeeId}' has not checked in today.", employeeId);
                return null;
            }

            if (attendance.CheckOutTime.HasValue)
            {
                _logger.LogInformation("CheckOut: Employee '{EmployeeId}' already checked out today.", employeeId);
                return null;
            }
        ...
        public async Task<AttendanceResponseDto?> MarkManualAttendanceAsync(ManualAttendanceDto dto, string createdBy)
        {
            ...
            var existingAttendance = await _attendanceRepository
                .GetByEmployeeAndDateAsync(dto.EmployeeId, dto.AttendanceDate.Date);
